...
function renderBag(){
 ...
 bagItems.innerHTML=state.bag.map(x=>`<div class="bag-item"><img src="${x.product.image}" alt="${x.product.name}"><div><strong>${x.product.name}</strong><small>${x.qty} × $${x.product.price.toFixed(2)}</small></div><button class="remove" data-remove="${x.product.id}">Remove</button></div>`).join('');
